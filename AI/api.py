...
# ---------------------------------------------------------------------------
def extract_user_preferences(data: Any) -> Dict[str, Any]:
    """
    Validate structured preference data received from the backend and build
    a semantic search query text from it.

    Accepts either an already-parsed dict (e.g. request.dict() from
    FastAPI/Pydantic) or a raw JSON string (e.g. when calling this function
    directly outside the API, such as in a test script).
    ...
    if isinstance(data, (str, bytes)):
        try:
            ...
            raise ValueError(f'data was a string but not valid JSON: {e}')

    if not isinstance(data, dict):
        raise ValueError(
            f'data must be a dict or a JSON string representing an object, got {type(data).__name__}.'
        )
...
def budget_allocation(
    ...
    - Multiple categories -> weights are treated as relative proportions
      (e.g. [2, 4, 4] means 20% / 40% / 40%) and normalized internally, so
      they do NOT need to sum to 1 beforehand.
      category_budget = total_budget * (category_weight / sum(weights))
    ...
    return {category: budget * weight for category, weight in zip(categories, normalized)}
...
- التوصية المقترحة لكل فئة (اسم المنتج، السعر لكل وحدة، المورّد، التقييم)
...
يرجى مراعاة أيضا:
- إذا لم تكن الفئة المطلوبة متوفرة قل :الفئة المطلوبة غير متوفرة حاليا بالنظام. يرجى اختيار فئة أخرى.
"""
...
async def ask(request: QueryRequest):
    ...
    try:
        prefs = extract_user_preferences(request.dict())
    ...
    return QueryResponse(response=extract_llm_text(llm_response))
