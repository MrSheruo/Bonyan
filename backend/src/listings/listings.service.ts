...
export async function getOwnStore(userId: string) {
  ...
  if (!store.verified)
    throw new ForbiddenError("Your store isn't verified yet");
  return store;
}
  ...
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (!productId) {
        ...
        const product = await insertProductWithImages(
          tx,
...
export async function restoreListing(id: number, requester: { id: string }) {
  ...
  if (!row) throw new NotFoundError("Listing not found");
  if (row.store.ownerId !== requester.id)
    throw new ForbiddenError("You don't own this listing");
...
export function withEffectivePrice(
  price: string,
  discount: { percentage: string } | null,
) {
  const base = Number(price);
  if (!discount) return { effectivePrice: base, hasDiscount: false };
  const effectivePrice = Number(
    (base * (1 - Number(discount.percentage) / 100)).toFixed(2),
  );
...
export async function getListingById(id: number) {
  const [row] = await db
    ...
    .innerJoin(stores, eq(listings.storeId, stores.id))
    .innerJoin(productImages, eq(listings.productId, productImages.productId))
    .where(and(eq(listings.id, id), isNull(listings.deletedAt)));

  if (!row) throw new NotFoundError("Listing not found");

  const [images, discount] = await Promise.all([
    getProductImages(row.product.id),
    getActiveDiscount(id),
  ]);
  return {
    ...row.listing,
    product: row.product,
...
export async function createDiscount(
  ...
  if (overlap) {
    throw new ConflictError(
      "This listing already has an active or upcoming discount",
    );
  }
...
export async function updateDiscount(
  ...
  const [existing] = await db
    ...
    .from(discounts)
    .where(
      and(eq(discounts.id, discountId), eq(discounts.listingId, listingId)),
    );
