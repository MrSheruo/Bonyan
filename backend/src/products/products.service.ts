import { eq, and, isNull, sql, ilike, getTableColumns, gte, desc } from "drizzle-orm";
...
import type { CreateProductInput, GetProductsQuery, UpdateProductInput } from "./products.validation.js";
...
async function assertCanManageProduct(_productId: string, requester: Requester) {
    ...
    if (!MANAGE_ROLES.has(requester.role)) {
        throw new ForbiddenError("You don't have permission to manage this product");
    }
}
...
export async function createProductWithImages(input: CreateProductInput, imageBuffers: Buffer[]) {
...
export async function updateProduct(id: string, input: UpdateProductInput, requester: Requester) {
...
export async function addProductImages(productId: string, imageBuffers: Buffer[], requester: Requester) {
    ...
    try {
        const [row] = await db
            .select({ maxSortOrder: sql<number>`coalesce(max(${productImages.sortOrder}), -1)` })
            .from(productImages)
...
export async function deleteProductImage(productId: string, imageId: number, requester: Requester) {
    await assertCanManageProduct(productId, requester);
...
export async function listProducts(query: GetProductsQuery) {
    const baseFilters = [
        ...
        query.minRating !== undefined ? gte(products.rating, String(query.minRating)) : undefined,
    ].filter((f): f is NonNullable<typeof f> => f !== undefined);

    const cols = getTableColumns(products);

    // ---------- search mode: relevance order ----------
    if (query.search) {
        ...
        let cursorCondition;
        if (query.cursor) {
            const { rank, id } = decodeCursor<{ rank: number; id: string }>(query.cursor);
        ...
        const rows = await db
            ...
            .from(products)
            .where(
                and(...baseFilters, sql`${products.searchVector} @@ ${tsquery}`, cursorCondition)
            )
        ...
        return {
            items: page.map(({ rank, ...p }) => p),
            nextCursor,
        };
    }

    // ---------- default mode: recent-bucket + rating order ----------
    const isRecentExpr = sql<boolean>`(${products.createdAt} >= now() - ${RECENT_WINDOW})`;

    let cursorCondition;
    if (query.cursor) {
        const { isRecent, rating, id } = decodeCursor<{ isRecent: boolean; rating: number; id: string }>(
            query.cursor
        );
        cursorCondition = sql`(
    ...
    const rows = await db
        ...
        .orderBy(sql`${isRecentExpr} DESC`, desc(products.rating), desc(products.id))
        .limit(PAGE_SIZE + 1);
    ...
    const last = page.at(-1);
    const nextCursor =
        hasMore && last ? encodeCursor({ isRecent: last.isRecent, rating: Number(last.rating), id: last.id }) : null;

    return {
        items: page.map(({ isRecent, ...p }) => p),
        nextCursor,
    };
}
