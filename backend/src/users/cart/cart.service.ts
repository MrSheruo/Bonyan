...
import { carts, cartItems, listings } from "@/db/schema.js";
import { NotFoundError, ConflictError } from "@/shared/errors.js";
import { getActiveDiscount, withEffectivePrice } from "@/listings/listings.service.js";
...
export async function getCartWithItems(userId: string) {
    ...
    const items = await db
        .select({ item: cartItems, listing: listings })
        .from(cartItems)
...
export async function addItemToCart(userId: string, listingId: number, quantity: number) {
    const cart = await getOrCreateActiveCart(userId);
